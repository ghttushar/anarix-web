...
import { useAccounts, ConnectedAccount, AccountGroup } from "@/contexts/AccountContext";
import { Marketplace } from "@/contexts/MarketplaceContext";
import { NavLink } from "react-router-dom";
import { Plus, ChevronRight } from "lucide-react";
import { AccountRegionPopup } from "./AccountRegionPopup";
...
export function MarketplaceHoverPopup({
  ...
  const [groupRects, setGroupRects] = useState<Record<string, DOMRect | null>>({});
  const regionHoverTimeoutRef = useRef<NodeJS.Timeout | null>(null);

  const filtered = accounts.filter((a) => a.marketplace === marketplace);
  const isAmazon = marketplace === "amazon";
  const groups = isAmazon ? accountGroups : [];
  ...
  const handleGroupMouseEnter = useCallback((groupId: string) => {
    if (regionHoverTimeoutRef.current) {
      clearTimeout(regionHoverTimeoutRef.current);
      regionHoverTimeoutRef.current = null;
    }
    const btn = groupRefs.current[groupId];
    if (btn) {
      setGroupRects((prev) => ({ ...prev, [groupId]: btn.getBoundingClientRect() }));
    }
    setHoveredGroupId(groupId);
  }, []);

  const handleGroupMouseLeave = useCallback(() => {
    regionHoverTimeoutRef.current = setTimeout(() => setHoveredGroupId(null), 200);
  }, []);

  const handleRegionPopupEnter = useCallback(() => {
    if (regionHoverTimeoutRef.current) {
      clearTimeout(regionHoverTimeoutRef.current);
      regionHoverTimeoutRef.current = null;
    }
  ...
  const handleRegionPopupLeave = useCallback(() => {
    setHoveredGroupId(null);
  }, []);

  useEffect(() => {
    return () => {
      if (regionHoverTimeoutRef.current) clearTimeout(regionHoverTimeoutRef.current);
    };
  }, []);
  ...
  const hoveredGroup = hoveredGroupId
    ...
    : null;
  const hoveredRegions = hoveredGroupId
    ? accountRegions.filter((r) => r.groupId === hoveredGroupId)
    : [];
  ...
  return createPortal(
    <>
      ...
      >
        ...
        {/* Account list — groups for Amazon, flat accounts for others */}
        <div className="p-1.5 max-h-[60vh] overflow-auto">
          {isAmazon ? (
            groups.length > 0 ? (
              groups.map((group) => {
                const groupRegions = accountRegions.filter((r) => r.groupId === group.id);
                const totalStatus = groupRegions.length > 0
                  ? groupRegions.some((r) => r.status === "error") ? "error"
                    : groupRegions.some((r) => r.status === "syncing") ? "syncing"
                    : "connected"
                  : "connected";
                return (
                  <div key={group.id} className="relative">
                    <button
                      ...
                      className={cn(
                        ...
                        groupRegions.some((r) => r.id === currentRegionId)
                          ? "bg-primary/10 text-primary font-medium"
                          : "text-popover-foreground hover:bg-muted hover:text-foreground"
                      )}
                    >
                      <StatusDot status={totalStatus as ConnectedAccount["status"]} />
                      <span className="truncate flex-1">{group.name}</span>
                      {groupRegions.length > 1 && (
                    ...
                    <AccountRegionPopup
                      ...
                      triggerRect={groupRects[group.id] || null}
                      currentRegionId={currentRegionId}
                      onSelectRegion={(regionId) => onSelectRegion?.(regionId)}
                      onMouseEnter={handleRegionPopupEnter}
                      onMouseLeave={handleRegionPopupLeave}
                    />
                  </div>
                );
              })
